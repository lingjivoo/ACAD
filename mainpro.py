...
parser.add_argument('--epoch', '-e', type=int, default=90, help='num of epochs')
...
# Training
def train(trainloader,net,criterion,optimizer,scheduler):
    net.train()
...
def main():
    ...
    for i_n in range(opt.start, opt.start+opt.times):
        ...
        if opt.drop_type == 'ACAD':
            net = ResNet18(num_classes=opt.num_classes,RGB_Image=opt.RGB_Image, drop='ACAD')
            print("Dropout: ACAD")
        else:
            net = ResNet18(num_classes=opt.num_classes,RGB_Image=opt.RGB_Image)
            print("Dropout: None")
        ...
        for epoch in range(start_epoch, total_epoch):
            ...
            train_loss = train(trainloader,net,criterion,optimizer,scheduler)
